...
class RepresentativeCreationBody(BaseModel):
    ...
    area_represented: str = Field(
        example="Nairobi",
        description="The area described under IEBC/Parliament as being represented by them",
    )
    phone_number: Optional[str] = Field(example="254711111111")
    image_source: Optional[str] = Field(
        example="https://www.parliament..."
    )  # Add to S3 metadata
    image_data_type: Optional[str] = Field(
        example="URL", description="BASE64_ENCODING/URL"
    )
    image_data: Optional[str] = Field(
        example="s3//....",
        descrption="It could be a url string or base_64 encoded string depending on the datatype specifed above",
    )
    gender: Optional[str] = Field(
        example="M", description="The gender of the respensantive(M/F/OTHER)"
    )
    representation_summary: Optional[Dict[str, Dict[str, str]]] = Field(
        {
            "0": {
                "duration": "2017/01-2022/01",
                "seat": "SENATOR",
                "area_represented": "NAIROBI",
            },
            "1": {
...
class RepresentativeUpdateBody(RepresentativeCreationBody):
    ...
    position: Optional[str] = (
        None  # Field(example='MP', description = 'MP/SENATOR/WOMEN REP/MCA' )
    )
    position_type: Optional[str] = (
        None  # Field(example='ELECTED ', description = 'ELECTED/nominated')
    )
    house: Optional[str] = (
        None  # Field(example="NATIONAL", description = 'NATIONAL/SENATOR')
    )
    area_represented: Optional[str] = (
        None  # Field(example = "Nairobi", description="The area described under IEBC/Parliament as being represented by them")
    )
