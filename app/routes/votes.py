...
from services.votes import (
    ...
    filter_votes, get_votes_files_list, 
    update_vote)
...
async def getVoteFiles(id, file_name:str):
    return await get_file_data(os.environ.get('VOTES_DATA_BUCKET_NAME'),
                               f'{id}/{file_name}')

@vote_router.get('/file/{id}/playback')
async def streamVoteFiles(id, 
                            start_KB:int, 
                            stop_KB:int, 
                            file_name:str 
                            ):
...
async def uploadVoteFiles( file_type: FileType, 
                                   fileUploadBody:FileUploadBody ):
    return await file_upload(os.environ.get('VOTES_DATA_BUCKET_NAME'),
                             file_type, 
